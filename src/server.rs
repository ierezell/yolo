...
use crate::protocol::*;
use crate::shared::{SharedPlugin, LocalWeaponFireEvent};
use bevy::prelude::*;
...
fn handle_new_client(
    ...
    commands.entity(client_entity).insert(ReplicationSender::new(
    ...
    let player_entity = commands.spawn(NetworkedPlayerBundle::new(
        player_id,
        Vec3::new(0.0, 2.0, 0.0),
    )).id();
    
    info!("Spawned player entity {:?} for client {:?}", player_entity, client_entity);
}
...
fn handle_weapon_fire(
    ...
    for (entity, mut receiver) in weapon_fire_receivers.iter_mut() {
        for weapon_fire in receiver.receive() {
            ...
            info!("Received weapon fire from client {:?}, player {}, position: {:?}, direction: {:?}", 
...
fn server_update_loop(time: Res<Time>) {
    ...
    unsafe {
        if current_time - LAST_LOG > 5.0 {
            info!("Server running - {} players connected, uptime: {:.1}s", 0, current_time);
...
fn convert_local_weapon_events(
    ...
    for event in local_events.read() {
        ...
        info!("Server processing weapon fire from player {}, position: {:?}, direction: {:?}", 
